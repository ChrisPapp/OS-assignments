...
void producer_init(struct producer *pd, struct theme *theme, int resource_1, int resource_2, int resource_3){
  ...
  if (pthread_mutex_init(&pd->short_max_time_mutex, NULL) != 0) {
		printf("Error in short_max_time_mutex initialization.");
	}
  if (pthread_mutex_init(&pd->short_sum_time_mutex, NULL) != 0) {
...
void producer_destroy(struct producer *pd) {
  ...
  pthread_mutex_destroy(&pd->short_max_time_mutex);
  pthread_mutex_destroy(&pd->short_max_time_mutex);
...
void producer_increment_short_time(struct producer *pd, int time) {
  pthread_mutex_lock(&pd->short_sum_time_mutex);
  pd->short_sum_time += time; // summarizing short_time
  pthread_mutex_unlock(&pd->short_sum_time_mutex);
}

void producer_check_if_short_time_max(struct producer *pd, int time) {
  pthread_mutex_lock(&pd->short_max_time_mutex);
	if (pd->short_max_time < time) {
		pd->short_max_time = time;
	}
	pthread_mutex_unlock(&pd->short_max_time_mutex);
}
